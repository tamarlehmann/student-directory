# We are opening and closing the files manually. Read the documentation of the File
# class to find out how to use a code block (do...end) to access a file, so that we
# don't have to close it explicitly (it will be closed automatically when the block
...
def load_students(filename = "students.csv") # method accepts the filename as an arguemnts. If not supplied
  if File.exist?(filename)
    ...
    puts "There is no record of the file"
    return
  end
end
